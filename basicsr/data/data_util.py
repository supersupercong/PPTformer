...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    assert len(folders) == 3, (
        'The len of folders should be 3 with [input_folder, input_mask_folder, gt_folder]. '
        f'But got {len(folders)}')
    assert len(keys) == 3, (
        'The len of keys should be 3 with [input_key, input_mask_key, gt_key]. '
        f'But got {len(keys)}')
    input_folder, input_mask_folder, gt_folder = folders
    input_key, input_mask_key, gt_key = keys

    input_paths = list(scandir(input_folder))
    input_mask_paths = list(scandir(input_mask_folder))
    gt_paths = list(scandir(gt_folder))

    assert len(input_paths) == len(input_mask_paths) == len(gt_paths), (
        f'{input_key} and {input_mask_key} and {gt_key} datasets have different number of images: '
        f'{len(input_paths)}, {len(input_mask_paths)}, {len(gt_paths)}.')
    paths = []
    for idx in range(len(gt_paths)):
        ...
        input_mask_path = input_mask_paths[idx]
        # print('input_mask_path', input_mask_path,input_path,gt_path)
        # basename_input_mask, ext_input_mask = osp.splitext(osp.basename(input_mask_path))
        input_name = f'{filename_tmpl.format(basename)}{ext_input}'
        input_path = osp.join(input_folder, input_name)
        assert input_name in input_paths, (f'{input_name} is not in '
                                           f'{input_key}_paths.')
        gt_path = osp.join(gt_folder, gt_path)
        input_mask_path = osp.join(input_mask_folder, input_name)

        # print('input_path-input_mask_path-gt_path',input_path,input_mask_path,gt_path)
        paths.append(
            dict([(f'{input_key}_path', input_path),
                  (f'{input_mask_key}_path', input_mask_path),
                  (f'{gt_key}_path', gt_path)]))
    return paths
