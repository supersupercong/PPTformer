...
def padding(img_lq, img_lq_mask, img_gt, gt_size):
    h, w, _ = img_lq.shape
    ...
    if h_pad == 0 and w_pad == 0:
        return img_lq, img_lq_mask, img_gt

    img_lq = cv2.copyMakeBorder(img_lq, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_lq_mask = cv2.copyMakeBorder(img_lq_mask, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_gt = cv2.copyMakeBorder(img_gt, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    # print('img_lq', img_lq.shape, img_gt.shape)
    if img_lq.ndim == 2:
        img_lq = np.expand_dims(img_lq, axis=2)
    if img_lq_mask.ndim == 2:
        img_lq_mask = np.expand_dims(img_lq_mask, axis=2)
    if img_gt.ndim == 2:
